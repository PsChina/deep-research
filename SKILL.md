...
- **不需要深度研究**：单点事实、不依赖时效数据、无外部信息需求的主观偏好选题 → 降级为快速回答
...
**Phase 1.25 和 Phase 1.5 永远不可跳过**——这是从 Phase 1 进入 Phase 2 的两道硬闸门。Phase 1.25 防「新生事物未入静态汇总源」（增量盲区），Phase 1.5 防「已知数据过期 + 实体覆盖不全」（存量盲区）。**两道闸门互补——Phase 1.25 查搜不到的新实体，Phase 1.5 查搜到的旧数据。**
...
2. 标注类型：[core]（影响中央判断）→ 派 2 个独立 sub-agent；[fill]/[verify] → 派 1 个。 **标签非静态**——后续轮次/阶段中新发现的证据若足以颠覆当前判断，原 [fill] 实体/方向必须立即升级为 [core]，追加独立派工深挖。主要矛盾与次要矛盾会随信息增量转化。
3.  同帧并行 spawn 所有 deep-research 工具调用
4. 综合返回 → 去重、交叉验证、更新框架
5. 判断边际收益 → 继续/收敛

** curiosity_questions 必须覆盖的维度（每轮至少各 1 条）**：
- **无偏探索**：描述性语言，不带产品名/公司名
- **维度穷举**：按 Phase 1 已发现的**每个分类维度**，穷举该维度下的实体，逐个搜其最新状态。 **维度清单从当期研究动态发现，不预设、不硬编码、不跨研究复用**。目的：防静态汇总源只覆盖部分维度/实体导致系统性遗漏
- **反方/失败案例**：主动搜索反对观点和负面评价
- ** 系统性偏见盲区检测（每轮必查，不可跳过）**：主动搜索以下偏见盲区——任何一种偏见都可能导致报告系统性失真：
  - **地理/语言偏见**：单一语言搜索 → 该语言生态外的实体系统性遗漏。任何有 ≥2 个重要独立实体的地理/语言生态，必须作为 [core] 方向独立派工
  - **开源 vs 商业偏见**：只搜开源侧或只搜商业侧 → 另一侧完全遗漏
  - **成熟 vs 新兴偏见**：汇总源天然偏向已建立声誉的老牌玩家，新兴实体被系统性低估
  - **分类框架偏见**：[core] vs [fill] 的标注本身可能引入偏见——如果某类实体的竞争力模型不匹配当前框架，可能被错误降级为 [fill]
  -  上述任一类偏见在 Phase 1 每轮至少产生 1 条主动探测 query
- ** 基础设施/底层引擎穷举（每轮必查，不可跳过）**：本主题的实体依赖哪些底层引擎/平台/协议/标准/运行时？这些底层能力提供者本身是否构成独立竞争维度？ **此项豁免「无产品名」约束**——允许直接使用引擎名/平台名/公司名狙击搜索。目的：防「产品视角」遗漏底层能力提供者（任何主题均存在「被依赖但未被独立追踪」的实体层——产品依赖引擎，引擎依赖协议，协议依赖标准）。
...
**为什么需要这道闸**：Phase 1 的 curiosity_questions 偏总结性，搜索引擎返回的是已有排名的汇总页面（排名/对比/目录/报告等），不是新闻式结果。Phase 1.5 只检查已知数据的时效，不探测未知实体的存在。二者都防不住「刚发布不久、尚未被汇总源收录的新实体」——这是新鲜度扫描的独立价值。
...
- 作为其他实体属性/依赖/组件/底层引擎出现的实体——提取被依赖方，不要只追踪顶层实体
- 被列为对比对象但未深度追踪的实体
- 在阶段性总结的子段落中提及但未展开的实体

**提取后整理为扁平实体清单**，不分子集、不预设重要性排序。每个实体在下述 4a/4b 中至少生成 1 条 query。

> 此步骤不引入任何领域术语——它是纯粹的实体提取操作，跨所有研究主题通用。

** 实体优先级分层（执行约束）**：扁平清单中的实体数常远超单轮 batch_search 容量（≤5 queries）。必须按以下优先级分配 query 槽位：

| 优先级 | 实体类型 | 分配策略 |
|---|---|---|
| **Tier 1（必须全覆盖）** | 可作为独立竞争维度出现的底层引擎/平台/协议/标准/运行时 | 每个实体至少 1 条 query，优先分配槽位 |
| **Tier 2（优先覆盖）** | 顶级产品/公司名（Phase 1 中被列为 [core] 的实体） | 尽可能覆盖，剩余槽位全部分配 |
| **Tier 3（尽力覆盖）** | 纯依赖/组件/被引用但未深度追踪的实体 | Tier 1+2 全部覆盖后，剩余槽位均分 |

若实体总数 > 单轮容量，允许多轮 batch_search 逐 tier 执行（Phase 1.25 最多 2 轮）。**Tier 1 实体绝不可因预算不足被跳过**——宁可砍 Tier 3 也不能漏 Tier 1。
...
4.  **逐实体狙杀式搜索（不可跳过，必须对 Phase 1 发现的每个实体生成至少 1 条 query）**：
   ...
   **4b.  后继探测（防「已知 X 但 X 已被替代」）**：
   Phase 1 发现某个实体 ≠ 该实体仍是最新/最优。必须主动探测是否存在后继者（替代品、升级版、新代际、新版本、更正或撤稿等）。**此项独立于 4a 预算——每发现一个版本化命名实体（含明确版本号/代际号/年份号），必须为其生成 N+1 探针 query，不可因槽位不足跳过。**
   - **命名可预测时**（实体有明确的版本号、代际号、年份号等递增标识）→ 按命名惯例生成 N+1、N+2 的精确 query 探针。**可合并到 4a 的 latest query 中节省槽位**：`"<实体名> latest OR <N+1> OR <N+2> release <current_month> <current_year>"`
   - **命名不可预测时**（实体无结构化版本标识）→ 4a 的「latest」query 为主力，辅以时间约束新闻搜索（`"<实体名> <current_month> <current_year>"`）
   - 发现后继存在 → 该实体标记为「已被替代」，立即 spawn 子 Phase 1 深挖后继
   - **核心原则**：不假设已知实体就是终点。找到什么不代表什么就是最新的。**漏掉版本化实体的后继探测 = Phase 1.25 不合格。**
...
10.  **系统性偏见自审（强制，五类偏见逐条过，不可敷衍）**：
    ...
    d. **分类框架偏见**：[core]/[fill] 标注是否引入了系统性歧视？——某类实体的竞争力模型若不匹配当前分类框架，可能被错误降级。 **新证据若足以颠覆 Central Thesis，原 [fill] 必须升级为 [core] 并追加独立派工**——矛盾的转化本身就是最重要的发现。
    e. **基准/评测偏见**：引用的基准是否只反映某一生态/某类产品的表现？→ 每个基准有其设计偏向（语言、任务类型、数据集来源），单一基准的排名不能代表全局能力
11.  **底层引擎/基础设施覆盖自审（强制）**：所有已发现的实体依赖哪些底层引擎/平台/协议/标准？是否遗漏了重要的底层能力提供者——特别是非主流生态/非英语信息源的实现？→ 若发现遗漏，spawn 子 Phase 1 补搜（豁免「无产品名」规则）。
...
### 5.0 实体回溯矩阵（强制首步，不可跳过）

**目标**：确保每个 Phase 1 + Phase 1.25 发现的 [core] 实体都映射到最终报告的至少一个章节。信息存在 ≠ 已写进报告。此步骤防「搜到了但写报告时漏掉」。

**流程**：
1. 从 Phase 1 findings + Phase 1.25 实体清单中提取**所有 [core] 实体**
2. 逐实体标注在最终报告中的映射位置（章节号）
3. 未映射的实体必须显式说明排除理由（如数据不足、置信度过低、与主题无关等）
4. 输出回溯表作为 Phase 5 第一步，之后再开始写报告正文
...
| 实体 | 发现于 | 报告映射位置 | 未映射原因 |
|------|--------|-------------|-----------|
| <实体名> | <Phase/Agent> | §X <章节名> | — |
| <实体名> | <Phase/Agent> | — | <理由> |
```

** 硬约束**：[core] 实体必须有归宿；[fill] 实体鼓励映射但不强制。回溯表不通过（有未映射且无理由的 [core] 实体）→ 补映射或补理由后再写正文。
...
| 12 | **防系统性偏见** | 五类偏见（地理/语言、开源vs商业、成熟vs新兴、分类框架、基准评测）每轮主动探测。多生态搜索是底线——只用一种语言/渠道 = 系统性遗漏其他生态。任何生态有 ≥2 个重要独立实体必须升级 [core] |
...
|  忽略系统性偏见——只用主导语言搜索、默认非主导生态实体是 [fill]、开源/商业只盯一侧、只信某一生态的基准 | Phase 1 每轮至少 1 条偏见盲区探测 query（非主导语言/偏门生态/新兴实体等）；Phase 2 换框自审第 10 条五类偏见逐条过；任何一类查出 ≥2 个重要遗漏实体 → 升级 [core] 独立派工 |
...
| 搜索 query 含具体产品/公司名 | 描述性语言——但 curiosity_questions 必须同时包含实体穷举 query |
...
|  [core]/[fill] 标签一成不变——新证据已显示某 [fill] 方向足以颠覆结论，仍按 [fill] 处理、不给独立派工 | 每轮综合后复查标签：新信息是否改变了各方向的相对重要性？矛盾转化时立即升级标签 + 追加派工 |
